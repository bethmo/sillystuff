	...
	<body>
		...
		<div id="list-pokerHand" class="content scaffold-list" role="main">
			...
			<table>
				<thead>
					<tr>
						<g:sortableColumn property="result" title="${message(code: 'pokerHand.test.label', default: 'Test')}" />
						...
						<g:sortableColumn property="actualDescription" title="${message(code: 'pokerHand.actualDescription.label', default: 'Actual Description')}" />
				...
				<g:each in="${pokerHandInstanceList}" status="i" var="pokerHandInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						<td>${fieldValue(bean: pokerHandInstance, field: "test")}</td>
						...
						<td>${fieldValue(bean: pokerHandInstance, field: "actualDescription")}</td>
